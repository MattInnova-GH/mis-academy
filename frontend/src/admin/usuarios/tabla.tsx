...
export default function UsuariosPage() {
  ...
  const toggleEstado = async (usuario: Usuario) => {
    ...
    try {
      const res = await fetch(
        ...
        `${API_URL}/admin/usuarios/${usuario.id_usuario}`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(
        `Usuario ${selectedUsuario.id_usuario ? "actualizado" : "creado"
        } con éxito`
      );
      setShowModal(false);
      fetchUsuarios();
...
¿Desea continuar?`
    ...
    const res = await fetch(
      `${API_URL}/admin/usuarios/${usuario.id_usuario}/force`,
            ...
            <tbody>
              {usuarios.map((u) => (
                <tr key={u.id_usuario} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${u.estado.toLowerCase() === "activo"
